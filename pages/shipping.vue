  ...
  <div class="container-fluid mtb-20 mtb-sm-15">
    <div class="product-detail checkout-detail">
      <div class="detail-left p-20 p-sm-15 pb-10 area mr-20 mr-sm mb-sm-15">
        ...
        <div class="flex align-start gap-15 start">
          <transition name="fade" mode="out-in">
            <div class="spinner-wrapper flex layer-white" v-if="loading">
              <spinner :radius="100" />
            </div>
          </transition>
                  ...
                  >
                    ...
                    <div class="icon-input">
                      ...
                      <input
                        type="text"
                        :placeholder="
                          $t('contact.your', { type: $t('contact.email') })
                        "
      ...
      <checkout-right
        ...
        :disabled="
          fetchingAddressData ||
          Object.keys(cartShipping).length === 0 ||
...
export default {
  ...
  computed: {
    selectedSippingPlace() {
      ...
      if (cartShippingValues?.length) {
        ...
        if (sp.pickup_phone) {
          addr = `${addr}, <span class="block">${this.$t("date.tl")}: ${
            sp.pickup_phone
          }</span>`;
        }
        return addr;
  ...
  methods: {
    ...
    selectCountry(evt) {
      this.addressData = {
        ...this.addressData,
        ...{ country: evt.value?.code2 },
      };
      this.states = evt.value?.states;
      this.addressData.state = Object.keys(evt.value?.states).length
        ? Object.values(evt.value?.states)[0]?.code
        : "";
    },
    selectState(evt) {
    ...
    settingCountry() {
      if (this.addressData) {
        if (
          ...
          this.countryList[this.location.countryCode]
        ) {
