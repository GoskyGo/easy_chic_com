<template>
  <div>
    ...
    <sticky-cart-btn :product-inventory="selectedInventory" :disabled="!statusPublic" :product="product"
      @cart-error="hasCartError" />
    <div v-if="product">

      <a v-if="product.store.whatsapp_btn" class="whatsapp-btn-wrap" target="_blank"
      ...
      <div class="container-fluid mtb-15 mt-sm-10 mn-h-400x">
        <div>
          <breadcrumb class="mb-20 mb-sm-15" :slugs="preparedSlug" :page="productTitle" />
          <div class="product-detail pr-0">
            <div class="detail-left">
              <div class="flex start align-start block-md">
                <div class="product-main">
                  <div class="detail-image-wrapper image-width">
                    <div class="detail-image-inner" :class="{ 'z-2': imagePopup }">
                      <product-images v-if="productImage || productImageList" ref="productImagesRef"
                        :title="productTitle" :product="product" :main-image="productImage" :images="productImageList"
                        @image-popup="imagePopup = $event" @add-to-wishlist="$refs.detailRight.wishListAction()" />
                    </div>
                  </div>
                  ...
                  <div class="pl-md grow mn-h-945px mx-w-450px">
                    ...
                    <h4 class="mb-15 bold" :class="[{ 'color-success': isInStock }, { 'color-danger': !isInStock }]">
                      {{ inStock }}
                    </h4>
                    ...
                    <div v-if="brand" class="two-sided mb-15">
                      ...
                      <div class="right">
                        <nuxt-link class="link" :to="brandLink(product.brand)">
                          {{ brand }}
                        </nuxt-link>
                      </div>
                    </div>
                    <div ref="attrRef"></div>
                    <div v-for="(value, index) in productAttributes" :key="index" class="two-sided mb-15">
                      <span class="left">
                        {{ value.title }}
                      </span>
                      <div class="start flex wrap gap-10">
                        <label v-for="(av, avIndex) in value.values" :key="`av-${avIndex}`"
                          class="rd-container rd-attr">
                          <input type="radio" :name="`${value.id}`" v-model="clickedAttributes[value.id]" :value="av.id"
                    ...
                    <div v-if="cartError.attribute" class="two-sided mb-15 align-start">
                      <h6 class="left">
                      </h6>
                      <div class="right">
                        <p class="error mb-10">
                    ...
                    <div v-if="vouchers && vouchers.length" class="two-sided mb-15 ">
                      <h6 class="left">
                        {{ $t('accountLayout.vouchers') }}
                      </h6>
                      <div class="pos-rel ">
                        <div class="right mlr--2-5 cp" data-ignore="voucher-pop-over"
                          @click.passive="voucherPopOver = !voucherPopOver">
                          <span v-for="(value, index) in vouchers" :key="index"
                            class="no-click info-msg ptb-5 mlr-2-5 mb-5 f-9">
                            {{ $t('detailRight.off', { amount: getPriceType(value) }) }}
                          </span>
                        </div>
                        <pop-over v-if="voucherPopOver" :title="$t('filter.shop')" @close="closeVoucherPopOver"
                          elem-id="voucher-pop-over" :layer="false">
                          <template v-slot:content>
                            <vouchers ref="voucherPagination" :changing-route="false" />
                          </template>
                        </pop-over>
                      </div>
                    </div>
                    ...
                    <div class="editor mt-30 mt-sm-15" v-dompurify-html="overview" />
                    <detail-right ref="detailRight" :product-inventory="selectedInventory" :disabled="!statusPublic"
                      :product="product" @cart-error="hasCartError" />
                  </div><!-- plr-30 grow -->

                </div><!-- flex -->
              </div>
              <!-- <client-only>
                <div class="ellipsis-para editor mt-30 mt-sm-15" :class="{ 'expanded': descriptionExpand }"
                  v-dompurify-html="description" />
                <button @click.prevent="descriptionToggle" aria-label="Read less" class="link mt-15 mb-5">
      ...
      <client-only>
        <div :class="{ 'mx-h-0': !hasReview, 'review-loaded': !reviewLoaded }"
          ...
          <lazy-area v-slot:default="{ renderArea }">
            <product-review v-if="renderArea" :id="product.id" class="b-t pt-20 pt-sm-15  "
...
export default {
  ...
  methods: {
    ...
    attrHover(value) {
      const attrVal = value.attribute_value_id;
      if (this.productAttributeImage[attrVal]) {
        this.$refs.productImagesRef.zoomActiveChange(this.productAttributeImage[attrVal].key + 1)
      }
    },
